...
class SnakeEnv(Environment[SnakeObservation, Direction]):
    ...
    def reset(self, seed: Optional[int] = None) -> Point:
        ...
        self.snake = [Point(floor(self.width/2), floor(self.height/2))] # snake always starts at the center of the level
        self.direction = Direction.LEFT
        self.apple = self.generate_random_apple()
    ...
    def actions(self) -> List[Direction]:
        ...
        for direction in Direction:
            if direction != self.direction: # check that action can not move the snake in the same direction

                # to check that action can not move the snake in the opposite direction of its current direction.
    ...
    def step(self, action: Direction) -> \
        ...
        if action != Direction.NONE: # check that direction is not none
            self.direction = action

        action = self.direction.to_vector() # get the action as coordinates i.e. (0,1)

        head = self.snake[0]
        new_head = head.__add__(action) # apply the action to the snake head
        new_head = Point(new_head.x % self.width, new_head.y % self.height) # make sure the head doesn't go out of the grid
        
        self.snake.insert(0, new_head) # push the new head, pop the tail to simuilate moving 1 step
        if new_head == self.apple: # if there's an apple, don't pop as the length of the snake will increase
            reward = 1   # Eating an apple increases the reward by 1.
            self.apple = self.generate_random_apple()
        else:
            self.snake.pop() 
        
        if new_head in self.snake[1:]: # if the new_head collides with the snake body, game is over
            done = True # set done as true (terminal state)
            reward = -100 # Losing the game decreases the reward by 100.
