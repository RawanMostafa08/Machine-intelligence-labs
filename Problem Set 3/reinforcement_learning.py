...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            max_Q = float('-inf')
            max_action = None
            for action in actions: #loop on actions to the action corresponding to maximum q-value
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        if next_action is None:
            ...
            nextQ = self.compute_q(env,next_state,next_action)
        # calculate the Q with bellman equation
        self.Q[state][action] += self.learning_rate * (reward + (self.discount_factor * nextQ) - self.compute_q(env,state,action)) 
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            max_next_q = self.compute_utility(env,next_state)

        # calculate the Q with bellman equation
        self.Q[state][action] += self.learning_rate * (reward + (self.discount_factor * max_next_q) - self.compute_q(env, state, action)) 
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        for feature in features:
            sum += features[feature] * self.weights[action].get(feature, 0) # calculate the sum of q values for all features
        return sum
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        # TODO: Complete this function
        return max(self.__compute_q_from_features(features, action) for action in self.weights) # get the maximum q value of all actions
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            # get q value for next state
            next_q_value = self.__compute_utility_from_features(next_features)

        for feature in features:
            # update the weight according to the equation
            self.weights[action][feature] += self.learning_rate * (reward + self.discount_factor * next_q_value - current_q_value) * features[feature]
